...
time_volume <- full_data_clean %>%
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ .,
                        name = "Media Volume (scaled 0–100)")
  ) +
...
All analyses presented in this report were conducted using R (version 4.5.1) within RStudio.  
The full analysis workflow, including data acquisition, cleaning, aggregation, visualisation, and statistical modelling, is fully reproducible using the code provided in this repository.

## Data sources

- Google Trends data was retrieved programmatically using the `gtrendsR` package. Due to limitations in reliably querying Google-defined topics via the unofficial API, search terms rather than Knowledge Graph topic identifiers were used. Google Trends data is provided as relative search interest values, normalised within each query.
- News media data was obtained from the GDELT 2.0 database using the Document API. Daily data were aggregated to weekly resolution to align with Google Trends outputs.

## Reproducibility considerations
Google Trends and GDELT are dynamic data sources that update continuously. As a result, exact numerical values may differ slightly if the analysis is re-run at a later date. However, the overall temporal patterns and analytical conclusions are expected to remain robust.

All data extraction dates are reported, and all transformations are explicitly documented in the analysis code. No manual data cleaning or undocumented pre-processing steps were performed.
...
Session information is provided below to support full reproducibility.
```{r session-info}
sessionInfo()
```
